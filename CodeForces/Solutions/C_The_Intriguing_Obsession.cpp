...
ll func(int a, int b)
{
  ...
  {
    result = mod_add(result, mod_mul(nCr(a, i, M), mod_mul(nCr(b, i, M), fact[i], M), M), M);
  }
  return result;
}
void solve()
{
  int a, b, c;
  cin >> a >> b >> c;
  fact_calc(max(a, max(b, c)), M);
  cout << mod_mul(func(a, b), mod_mul(func(b, c), func(c, a), M), M) << endl;
}
