...
void solve()
{
  ...
  for (int i = 0; i < n; i++)
  {
    if (rooms.empty())
    {
      rooms.push({arr[i].first.second, curr + 1});
      curr++;
      ans[arr[i].second] = curr;
    }
    else
    {
      pair<int, int> p = rooms.top();
      if (p.first < arr[i].first.first)
      {
        rooms.pop();
        rooms.push({arr[i].first.second, p.second});
        ans[arr[i].second] = p.second;
      }
      else
      {
        rooms.push({arr[i].first.second, curr + 1});
